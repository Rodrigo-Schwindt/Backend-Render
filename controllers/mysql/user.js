...
      }
      // Nota: Eliminamos el log de payload aquí, ya que no es necesario en producción.
...
      console.error("Error en resend verification:", e);
      ...
      // 🚨 CAMBIO CLAVE: Manejo explícito del error "Usuario ya verificado"
      if (e.message && e.message.includes('ya está verificado')) {
          return res.status(409).json({ message: "Este usuario ya ha sido verificado." });
      }

      return res.status(500).json({ message: 'Error en el servidor al intentar reenviar.' });
    }
  };
